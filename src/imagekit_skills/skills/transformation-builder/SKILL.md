...
description: "Build and apply image/video transformations including AI-powered editing (change objects, colors, styles), background removal/replacement, generative fill, upscaling, retouching, resize, crop, overlays, text overlays, blur, sharpen, rotate, borders, shadows, color replace, and all other visual modifications. Use this skill whenever the user wants to edit, modify, transform, enhance, or manipulate any image or video."
---

# Transformation Guidelines Skill
...
## Using transformation_builder_tool

Call with:
- `query`: Natural language description (be specific about dimensions, focus, effects)
- `src`: Optional source URL (defaults to sample image)

**Chain transformations**: Frame multi-step requests as single query.
...
1. **400 / Bad Request**: Search docs via `search-doc` tool for parameters/limits
2. **Multiple failures (3+)**: Use `search-doc` tool to find supported methods and constraints
